"""Enforcement test (cross-repo issue #187 / #49 convention): hand-authored
...
decomposed "h"/"H" + COMBINING DOT BELOW (U+0323) sequence. Comments must not
use either Unicode form at all -- plain ASCII "x"/"X" is used instead, since
...
stdlib, so it runs under any interpreter from the repo dir.
"""
import subprocess
import unicodedata
import unittest
...
class TestHDotBelowNfc(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # files (catches a badly broken exclusion filter). This repo has ~27
        # hand-authored tracked text files; 20 is a comfortable floor below it.
        assert len(cls.in_scope_files) > 20, (
